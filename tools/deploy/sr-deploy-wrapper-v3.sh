  ...
  case "$requested" in
    current_dev)
      ...
      if [[ -d "$payload_root/022c2_stable" ]]; then
        echo "SR_BRIDGE: resolving payload alias current_dev -> 022c2_stable (stable fallback)" >&2
        printf '%s\n' "022c2_stable"
        return 0
      fi
      ;;
...
if [[ ! -x "$APPLY" || ! -x "$HEALTH" || ! -x "$REMOVE" ]]; then
  echo "SR_DEPLOY: missing deploy candidate scripts for component=$COMPONENT under $BASE" >&2
  exit 2
fi
